...
datasets = {
    '10percent': {
        'vx': pd.read_csv('./filtered10percent_vx_0to500.csv'),
        'vy': pd.read_csv('./filtered10percent_vy_0to500.csv'),
        'syy': pd.read_csv('./filtered10percent_syy_0to500.csv'),
        'ent': pd.read_csv('./filtered_ent_0to500.csv')
    },
    '20percent': {
        'vx': pd.read_csv('./filtered20percent_vx_0to500.csv'),
        'vy': pd.read_csv('./filtered20percent_vy_0to500.csv'),
        'syy': pd.read_csv('./filtered20percent_syy_0to500.csv'),
        'ent': pd.read_csv('./filtered_ent_0to500.csv')
    }
}
...
app.layout = html.Div([
    html.Div([
        html.H1("Interactive 3D plots of C50 for De = 9 during the extension: velocity (Vx, Vy), Stress (Syy), and crystal clusters", 
                style={'color': '#2C3E50', 'textAlign': 'center', 'backgroundColor': '#AED6F1', 'padding': '10px', 'borderRadius': '5px'}),
        
        html.Div([
            html.Div([
                html.P("To update the visualization, please control the time slider (min = 0 and max = 2 in Hencky strains):", 
                ...
                dcc.Slider(
                    ...
                    step=1,
                    value=0,
                    marks={i: str(i) for i in range(0, 500, 10)},
                    updatemode='drag'
                ),
        ...
        html.Div([
            ...
            html.Ul([
                ...
                html.Li("De and Hencky strain is reported for the C50 relaxation time = 0.75 ns")
            ]),
            dcc.Textarea(
                id='notes',
...
def update_plots(step, data_series):
    ...
    def create_3d_scatter(data, col):
        ...
        fig.update_layout(scene_camera=camera, 
                          scene=dict(
                              aspectmode='manual',
                              aspectratio=dict(x=x_dim*3.5, y=y_dim*3.5, z=z_dim*8)
                          ))
        return fig
